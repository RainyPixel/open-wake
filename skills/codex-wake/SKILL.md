...
Use `codex-wake` to end the current turn and resume it once a local condition is
true or its deadline is reached.

## Workflow

1. Start the long-running work under an independent supervisor or other durable
   execution mechanism. `codex-wake` observes work; it does not keep an ordinary
...
5. When the `codex-wake` continuation arrives, inspect its success, timeout, or
...
Use `codex-wake status` to diagnose an armed condition and `codex-wake cancel`
to release a waiting turn without continuing it. Only one condition may be
active per Codex session.

If setup is missing or stale, run `codex-wake doctor`. Apply the exact
`codex-wake setup --scope user|project` command it recommends, then review and
